...
execute as @a[scores={exp_hc=1}] at @s run tellraw @s[scores={load=60}] {"rawtext":[{"text":"[実験的機能] ホリデークリエイターの機能:§a有効"}]}
execute as @a[scores={exp_hc=1}] at @s run playsound random.levelup @s[scores={load=60}] ~ ~ ~ 2 1.8
execute as @a[scores={exp_hc=0}] at @s run tellraw @s[scores={load=60}] {"rawtext":[{"text":"[実験的機能] ホリデークリエイターの機能:§c無効"}]}
execute as @a[scores={exp_hc=0}] at @s run playsound random.toast @s[scores={load=60}] ~ ~ ~ 1

execute as @a[scores={exp_hc=0}] at @s run tellraw @s[scores={load=60}] {"rawtext":[{"text":"[エラー] ホリデークリエイターの機能が§c無効§rです。正常な動作にはホリデークリエイターの機能が有効である必要があります。"}]}
